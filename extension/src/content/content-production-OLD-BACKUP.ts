...
class ArborExtensionProduction {
  ...
  private onPageReady() {
    ...
    // Watch for sidebar removal and re-inject
    ...
    setTimeout(() => {
      ...
      if (this.availableChats.length === 0) {
        setTimeout(() => {
          this.availableChats = this.currentPlatform.getAllChatsFromSidebar();
          console.log(
            `📚 Retry: Found ${this.availableChats.length} available chats`
          );
          if (this.sidebarInjected) {
            this.refresh();
          }
  ...
  private showTrackChatPrompt(chatId: string, url: string, title: string) {
    ...
    // Event listeners
    document
      .getElementById("arbor-track-yes")
  ...
  private injectToggleButtons() {
    ...
    leftToggle.querySelector("button")?.addEventListener("click", () => {
      ...
      if (sidebar) {
        ...
        (leftToggle.querySelector("button") as HTMLElement).style.display =
          "none";
        this.adjustMainContent();
      }
    });
    ...
    rightToggle.querySelector("button")?.addEventListener("click", () => {
      ...
      if (graph) {
        ...
        (rightToggle.querySelector("button") as HTMLElement).style.display =
          "none";
        this.adjustMainContent();
      }
    });
  ...
  private injectStyles() {
    ...
    style.textContent = `
      ...
      .untracked-chat-item:hover {
        background: #232323 !important;
        border-color: rgba(45, 212, 167, 0.3) !important;
        color: #9caba3 !important;
      }

      .untracked-chat-item:hover .add-to-tree-btn {
        background: rgba(45, 212, 167, 0.2) !important;
      ...
      .btn:hover {
        background: linear-gradient(135deg, #5eead4 0%, #2dd4a7 100%);
        transform: translateY(-1px);
        box-shadow: 0 4px 12px rgba(45, 212, 167, 0.3);
      }
  ...
  private getSidebarHTML(): string {
    ...
    const untrackedChats = this.availableChats.filter(
      (chat) => !trackedUrls.has(chat.url)
    );
      ...
      <div style="padding: 16px 20px;">
        ...
        ${allTrees
          .map((tree) => {
            ...
            const branchCount = nodeCount > 1 ? nodeCount - 1 : 0; // root doesn't count as branch
            return `
    ...
    const currentTreeHTML =
      this.state.currentTreeId && this.state.trees[this.state.currentTreeId]
        ? `
      <div style="padding: 16px 20px; border-top: 1px solid #2a3530;">
        ...
        ${this.renderTreeNode(
          this.state.trees[this.state.currentTreeId].rootNodeId,
      ...
      <div style="padding: 16px 20px; border-top: 1px solid #2a3530;">
        ...
        <div style="max-height: 250px; overflow-y: auto; padding-right: 4px;">
          ${untrackedChats
            ...
            .map(
              (chat, index) => `
            <div class="untracked-chat-item" data-chat-url="${
              chat.url
            }" data-chat-index="${index}" style="
              background: #1c1c1c;
              padding: 10px 12px;
              margin-bottom: 6px;
              border-radius: 6px;
          ...
          `
            ...
            .join("")}
          ${
            untrackedChats.length > 15
              ? `<div style="color: #6a7570; font-size: 10px; padding: 8px 4px; opacity: 0.7;">...${
                  untrackedChats.length - 15
                } more chats</div>`
              : ""
          }
        </div>
        <div style="color: #6a7570; font-size: 10px; margin-top: 10px; opacity: 0.7;">
          Click to visit • Click + to add
        </div>
      </div>
    `
        : "";
    ...
    return `
      ...
      <div class="action-buttons" style="padding-top: 12px;">
        <button class="btn" id="new-chat-btn" title="Create a new ChatGPT chat">
          <span style="margin-right: 6px;">✨</span> New Chat
        </button>
      </div>
      <div class="action-buttons" style="padding-top: 0; padding-bottom: 20px; gap: 10px;">
          ...
          <div class="tree-node-meta">
            ...
            ${
              node.connectionLabel
                ? `<span>• ${node.connectionLabel}</span>`
                : ""
            }
          </div>
        </div>
    ...
    Object.values(tree.nodes).forEach((node) => {
      if (node.parentId && positions[node.parentId] && positions[node.id]) {
        ...
        line.addEventListener("click", async (e) => {
          e.stopPropagation();
          await this.editConnectionLabelDirect(node.id, node.parentId!);
        });
        ...
        // Add connection label button or existing label
        if (node.connectionLabel) {
          ...
          label.style.border = "1px solid rgba(45, 212, 167, 0.4)";
          label.style.borderRadius = "12px";
          label.style.padding = "4px 10px";
          label.style.fontSize = "11px";
          label.style.color = "#2dd4a7";
          ...
          addButton.style.left = `${midX}px`;
          addButton.style.top = `${midY}px`;
          addButton.style.transform = "translate(-50%, -50%)";
          addButton.style.background = "rgba(28, 36, 32, 0.9)";
          addButton.style.border = "1px solid #2a3530";
          addButton.style.borderRadius = "10px";
          addButton.style.padding = "4px 8px";
          addButton.style.fontSize = "10px";
          addButton.style.color = "#6a7570";
          ...
          addButton.style.transition = "all 0.15s ease";
          ...
          addButton.addEventListener("mouseleave", () => {
            addButton.style.background = "rgba(28, 36, 32, 0.9)";
            addButton.style.borderColor = "#2a3530";
            addButton.style.color = "#6a7570";
  ...
  private renderGraphNodes(
    ...
    Object.entries(positions).forEach(([nodeId, pos]) => {
      ...
      // Make node draggable and track movement
      const dragState = this.makeNodeDraggable(nodeEl, nodeId);

      // Left click - navigate (only if didn't drag)
      nodeEl.addEventListener("click", (e) => {
        if (
          !dragState.hasMoved &&
          !(e.target as HTMLElement).closest(".graph-node-title")
        ) {
  ...
  private showCustomizationPanel(nodeId: string) {
    ...
    panel.innerHTML = `
      ...
      ">
        <div style="padding: 20px; border-bottom: 1px solid #333;">
          ...
          <p style="color: #999; margin: 8px 0 0 0; font-size: 13px;">${
            node.title
          }</p>
        </div>
          ...
          <div style="margin-bottom: 20px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
              <button class="shape-btn" data-shape="rounded" style="
                padding: 12px;
                background: ${
                  currentShape === "rounded" ? "#4a9eff" : "#252525"
                };
                border: 1px solid #333;
                border-radius: 8px;
                color: #fff;
              ...
              <button class="shape-btn" data-shape="rectangle" style="
                padding: 12px;
                background: ${
                  currentShape === "rectangle" ? "#4a9eff" : "#252525"
                };
                border: 1px solid #333;
                border-radius: 0px;
                color: #fff;
              ...
              <button class="shape-btn" data-shape="circle" style="
                padding: 12px;
                background: ${
                  currentShape === "circle" ? "#4a9eff" : "#252525"
                };
                border: 1px solid #333;
                border-radius: 50%;
                color: #fff;
              ...
              <button class="shape-btn" data-shape="diamond" style="
                padding: 12px;
                background: ${
                  currentShape === "diamond" ? "#4a9eff" : "#252525"
                };
                border: 1px solid #333;
                border-radius: 4px;
                color: #fff;
    ...
    panel.querySelectorAll(".shape-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        selectedShape =
          ((btn as HTMLElement).dataset.shape as ChatNode["shape"]) ||
          "rounded";
    ...
    // Cancel button
    document
      .getElementById("cancel-customization")
      ?.addEventListener("click", () => {
        panel.remove();
      });
    ...
    setTimeout(() => {
      const closeOnOutside = (e: MouseEvent) => {
        if (
          !(e.target as HTMLElement).closest("#arbor-customization-panel > div")
        ) {
          panel.remove();
  ...
  private async renameNode(nodeId: string) {
    ...
    try {
      const renamed = await this.currentPlatform.renameChat(
        node.url,
        newTitle.trim()
      );
      if (renamed) {
        ...
        this.showNotification(
          "Renamed in Arbor (ChatGPT rename not available)",
          "success"
        );
      }
    } catch (error) {
  ...
  private async editConnectionLabel(nodeId: string) {
    ...
    await this.editConnectionLabelDirect(nodeId, node.parentId);
  }

  private async editConnectionLabelDirect(
    childNodeId: string,
    parentNodeId: string
  ) {
    if (!this.state.currentTreeId) return;
    ...
    await db.saveTree(tree);
    await db.saveNode(childNode, this.state.currentTreeId);
    this.renderGraph();
    this.refresh();
    this.showNotification(
      `Label updated: ${parentNode.title} → ${childNode.title}`,
      "success"
    );
  }
    ...
    if (this.state.currentTreeId === treeId) {
      ...
      if (remainingTrees.length > 0) {
        ...
        this.state.currentNodeId =
          this.state.trees[remainingTrees[0]]?.rootNodeId || null;
      } else {
  ...
  private async deleteNode(nodeId: string) {
    ...
    if (nodeId === tree.rootNodeId) {
      this.showNotification(
        "Cannot delete root node! Delete the entire tree instead.",
        "error"
      );
      return;
    }
    ...
    this.refresh();
    this.showNotification(
      `Deleted ${totalToDelete} node${totalToDelete !== 1 ? "s" : ""}! 🗑️`,
      "success"
    );
  }
  ...
  private attachSidebarListeners() {
    document.getElementById("toggle-sidebar")?.addEventListener("click", () => {
      const sidebar = document.getElementById("arbor-sidebar-container");
      const leftToggle = document
        .getElementById("arbor-left-toggle")
    ...
    // Tree title editing
    document
      .getElementById("tree-title-editable")
      ?.addEventListener("click", () => {
        this.editTreeTitle();
      });
    ...
    document.querySelectorAll(".tree-item").forEach((item) => {
      ...
      const treeId = container
        ? (container as HTMLElement).dataset.treeId
        : null;
    ...
    document
      .getElementById("new-tree")
      ?.addEventListener("click", () => this.createNewTree());
  ...
  private attachGraphListeners() {
    document.getElementById("toggle-graph")?.addEventListener("click", () => {
      const graph = document.getElementById("arbor-graph-container");
      const rightToggle = document
        .getElementById("arbor-right-toggle")
        ?.querySelector("button") as HTMLElement;
    ...
    if (canvas) {
      ...
      canvas.addEventListener("mousedown", (e) => {
        ...
        if (
          (e.target as HTMLElement).id === "graph-canvas" ||
  ...
  private async createBranch() {
    ...
    const context = this.currentPlatform.generateBranchContext({
      ...
      summary: `Recent context: ${recentMessages
  ...
  private showChatBrowser() {
    ...
    if (allChats.length === 0) {
      this.showNotification(
        "No chats found in sidebar. Try scrolling to load more chats.",
        "error"
      );
      return;
    }
    ...
    // Close handlers
    modal
      .querySelector("#close-chat-browser")
      ?.addEventListener("click", () => {
        modal.remove();
      });

    modal
      .querySelector("#cancel-chat-browser")
      ?.addEventListener("click", () => {
        modal.remove();
      });
  }
  ...
  private async switchToTree(treeId: string) {
    ...
    this.showNotification(
      `Switched to tree: ${this.state.trees[treeId]?.name}`,
      "success"
    );
    this.refresh();
  }
  ...
  private async createNewTree() {
    ...
    this.showNotification(
      `Tree "${title}" will be created when you add your first chat.`,
      "success"
    );
    this.refresh();
  }
  ...
  private setupSidebarObserver() {
    ...
    // Observe the entire body for child node removals
