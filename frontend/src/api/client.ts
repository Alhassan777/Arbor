...
export const api = {
  ...
  async sendMessage(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Include userMessage if backend returned it (happens when error occurs after message is saved)
        if (error.userMessage) {
          apiError.userMessage = error.userMessage;
        }
        throw apiError;
      }
      return response.json();
    } catch (error) {
      // Handle network errors (fetch fails completely - no response.status available)
      if (error && typeof error === "object" && "statusCode" in error) {
        // Already formatted error from above
        throw error;
      }
      // Network error or other fetch failures
      const networkError: { message: string; statusCode: number } = {
  ...
  async summarizeConversation(conversationId: string): Promise<string> {
    const response = await fetch(
      `${API_BASE}/conversation/${conversationId}/summarize`,
      {
        method: "POST",
      }
    );
